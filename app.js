...
// Parser dédié au format PUM
function parsePumLines(rawLines) {
  const lines = [];
  // On essaie plusieurs largeurs de code et on garde le meilleur match.
  const patterns = [
    { re: /^\s*(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(.+?)(\d{2}-\d{4})\s*(Pi[èe]ce|M[èe]tre|Lot|Forfait|Kg|G|L|Carton|Bo[îi]te|Sachet|U)\s*(\d+(?:[\.,]\d{1,4})?)\s*$/i, codeLen: 7 },
    { re: /^\s*(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(.+?)(\d{2}-\d{5})\s*(Pi[èe]ce|M[èe]tre|Lot|Forfait|Kg|G|L|Carton|Bo[îi]te|Sachet|U)\s*(\d+(?:[\.,]\d{1,4})?)\s*$/i, codeLen: 8 },
    { re: /^\s*(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(.+?)(\d{3}-\d{5})\s*(Pi[èe]ce|M[èe]tre|Lot|Forfait|Kg|G|L|Carton|Bo[îi]te|Sachet|U)\s*(\d+(?:[\.,]\d{1,4})?)\s*$/i, codeLen: 9 },
    { re: /^\s*(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(\d+[\.,]\d{1,4})\s+(.+?)(\d{4}-\d{5})\s*(Pi[èe]ce|M[èe]tre|Lot|Forfait|Kg|G|L|Carton|Bo[îi]te|Sachet|U)\s*(\d+(?:[\.,]\d{1,4})?)\s*$/i, codeLen: 10 },
  ];

  // Score d'un match : meilleur = désignation qui se termine par une lettre/cm/dimension
  // (donc PAS un chiffre seul qui appartiendrait au code)
  function scoreMatch(desig) {
    const trimmed = desig.trim();
    if (!trimmed) return -1;
    const lastChar = trimmed.slice(-1);
    // Bonus : termine par une lettre = bien coupé
    if (/[A-Za-zÀ-ÿ]/.test(lastChar)) return 10;
    // Bonus moyen : termine par un chiffre suivant une virgule (dimension type "X,8")
    if (/,\d$/.test(trimmed)) return 5;
    // Termine par un chiffre seul = mal coupé (chiffre du code mangé)
    return 0;
  }
  ...
  for (const raw of rawLines) {
    ...
    for (const pat of patterns) {
      ...
      if (!m) continue;
      const puNet = parseFrNumber(m[1]);
      const puUnit = parseFrNumber(m[2]);
      const montant = parseFrNumber(m[3]);
      const desig = m[4].trim();
      const qty = parseFrNumber(m[7]);
      if (isNaN(puNet) || isNaN(qty) || isNaN(montant)) continue;
      // Cohérence : qty * puUnit ≈ montant
      if (Math.abs(qty * puUnit - montant) > Math.max(0.05, montant * 0.02)) continue;
      const s = scoreMatch(desig);
      if (s > bestScore) {
        bestScore = s;
        bestMatch = { puNet, puUnit, montant, desig, code: m[5], unit: m[6] || 'U', qty };
      }
    }
    if (!bestMatch) continue;

    let desig = bestMatch.desig.replace(/[\s\-:]+$/, '').replace(/\s+/g, ' ').trim();
    if (desig.length < 3) continue;

    let unitNorm = bestMatch.unit.toLowerCase();
    if (unitNorm === 'pièce' || unitNorm === 'piece') unitNorm = 'Pièce';
    else if (unitNorm === 'mètre' || unitNorm === 'metre') unitNorm = 'ML';
    else unitNorm = bestMatch.unit.toUpperCase();
...
function openInvoiceModal(id) {
  ...
  // Pièces jointes (PDF stocké)
  const attachmentsHtml = (inv.attachments && inv.attachments.length) ? inv.attachments.map((a, i) => {
    ...
    return `
      <div class="attachment-item">
        ...
        <div style="flex:1">
          ...
          <div class="muted small">${sizeKb} Ko · ${escapeHtml(a.type || 'fichier')}</div>
        </div>
        <button class="btn btn-secondary btn-mini" data-viewatt="${i}">Voir</button>
        <a class="btn btn-secondary btn-mini" href="${a.data}" download="${escapeAttr(a.name)}">Télécharger</a>
      </div>`;
  }).join('') : '<div class="empty-card" style="padding:16px;font-size:12px">Aucun fichier joint.</div>';
  ...
  $('#invoiceModalBody').innerHTML = `
    ...
    <div class="account-section">
      <h4>Pièces jointes ${(inv.attachments && inv.attachments.length) ? `(${inv.attachments.length})` : ''}</h4>
  ...
  // Bouton "Voir" pour ouvrir une pièce jointe
  $('#invoiceModalBody').querySelectorAll('[data-viewatt]').forEach(b =>
    b.addEventListener('click', () => {
      const idx = parseInt(b.dataset.viewatt);
      const att = (inv.attachments || [])[idx];
      if (att && att.data) {
        const w = window.open();
        if (w) {
          w.document.write(`
            <html><head><title>${escapeHtml(att.name)}</title>
            <style>body{margin:0;background:#222} iframe,img{width:100vw;height:100vh;border:0;display:block;object-fit:contain}</style>
            </head><body>
            ${(att.type || '').includes('pdf') || att.name.toLowerCase().endsWith('.pdf')
              ? `<iframe src="${att.data}"></iframe>`
...
async function handleFiles(files) {
  ...
  for (const f of files) {
    ...
    allText += '\n' + t;
    // Stocker le PDF/image en base64 pour consultation ultérieure
    try {
      ...
      console.warn('Impossible de stocker le fichier', f.name, err);
    }
  }
  ...
  toast(`${parsed.lines.length} lignes détectées · PDF stocké`, 'ok');
}

// Convertit un fichier en base64 (data:URL)
